...
int DFT(int idft, double *xr, double *xi, double *Xr_o, double *Xi_o, int N, int p);
...
int main(int argc, char *argv[])
{
    ...
    for(int j=0; j<lp; j++)
    {
        ...
        // DFT
        int idft = 1;
        DFT(idft, xr, xi, Xr_o, Xi_o, N, p[j]);
        // IDFT
        idft = -1;
        DFT(idft, Xr_o, Xi_o, xr_check, xi_check, N, p[j]);

        // stop timer
        double run_time = omp_get_wtime() - start_time;
        printf("Number of threads p = %d   ", p[j]);
                 ...
                 double *Xr_o, double *Xi_r, int N)
{
    ...
    for (int n = 0; n < N; n++)
    {
        if (fabs(xr[n] - xr_check[n]) > R_ERROR)
            printf("ERROR r - x[%d] = %f, inv(X)[%d]=%f \n", n, xr[n], n, xr_check[n]);
        if (fabs(xi[n] - xi_check[n]) > R_ERROR)
            printf("ERROR i - x[%d] = %f, inv(X)[%d]=%f \n", n, xi[n], n, xi_check[n]);
    }
    printf("Xre[0] = %f \n", Xr_o[0]);
    return 1;
}
