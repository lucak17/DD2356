...
int main(int argc, char *argv[])
{
    ...
    MPI_Irecv(&f[0],1,MPI_DOUBLE,prev,0,MPI_COMM_WORLD,&request_left[0]);
    MPI_Isend(&f[nxn_loc-3],1,MPI_DOUBLE,next,0,MPI_COMM_WORLD,&request_left[1]);
    MPI_Waitall(2,request_left,MPI_STATUS_IGNORE);

    MPI_Irecv(&f[nxn_loc-1],1,MPI_DOUBLE,next,0,MPI_COMM_WORLD,&request_right[0]);
    MPI_Isend(&f[2],1,MPI_DOUBLE,prev,0,MPI_COMM_WORLD,&request_right[1]);
    MPI_Waitall(2,request_right,MPI_STATUS_IGNORE);
